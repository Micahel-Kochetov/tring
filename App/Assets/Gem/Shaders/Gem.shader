...
Shader "FX/Gem"
{
	Properties {
		...
		_RefractScaleX("_RefractScaleX", Float) = 1
		_RefractScaleY("_RefractScaleY", Float) = 1
		_RefractScaleZ("_RefractScaleZ", Float) = 1
	}
	SubShader {
		Tags {
			"Queue" = "Transparent"
		...
		Pass {
			...
			half _Emission;
			float _RefractScaleX;
			float _RefractScaleY;
			float _RefractScaleZ;

			half4 frag (v2f i) : SV_Target
			{
				half3 refraction = texCUBE(_RefractTex, float3(i.uv.x*_RefractScaleX, i.uv.y*_RefractScaleY, i.uv.z*_RefractScaleZ)).rgb * _Color.rgb;
